# -*- coding: utf-8 -*-
"""
Created on Wed May 28 19:49:11 2025
...
def normalize_band(band, low=2, high=95, gamma=1.0):
...
output_dir = r"C:\Users\Moi\Desktop\tfg\videoimatgesvilanova\10juriol2018\imatgesEPGicortad"
...
with rasterio.open(output_tif) as image:
    bands = [image.read(i + 1).astype(np.float32) for i in range(4)]  # B, G, R, NIR
...
for i, name in enumerate(band_names):
    # Aplicar millora de lluminositat a Red i NIR
    if name in ["NIR"]:
        norm = normalize_band(bands[i], low=0.5, high=94, gamma=0.8)
    else:
        norm = normalize_band(bands[i])
    norm_path = os.path.join(output_dir, f"{name}.png")
    cv2.imwrite(norm_path, norm)
    print(f" Banda normalitzada: {norm_path}")

    # Retallar vores negres
    cropped = crop_black_borders(norm)
    cropped_path = os.path.join(output_dir, f"{name}_cropped.png")
    cv2.imwrite(cropped_path, cropped)
    print(f" Banda retallada: {cropped_path}")
    normalized_bands.append(cropped)

#  Igualar dimensions
min_height = min(b.shape[0] for b in normalized_bands)
min_width = min(b.shape[1] for b in normalized_bands)
resized_bands = [cv2.resize(b, (min_width, min_height)) for b in normalized_bands]

#  Crear composició RGB (Red = 3, Green = 2, Blue = 1)
